...
with tab4:
    ...
    # Format data labels based on user selection
    if label_format == "Actual Values":
        y_data['Data Labels'] = y_data[y_axis].apply(lambda x: f'{x:,.0f}')
    else:
        y_data['Data Labels'] = y_data[y_axis].apply(format_large_numbers)
